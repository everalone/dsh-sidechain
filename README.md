...
- 每行显示：子代理标题、类型（`/side 可续聊` / `/btw 一次性`）、运行状态（绿点运行中/灰点已结束）
- 点击行 = 跳转到该侧会话视图（同目录按钮的 `openSubagent` 行为）
- 面板打开时自动**订阅并刷新**子代理目录：新子代理出现、运行结束都会实时更新；标题行按钮上有**运行中数量角标**
- `/side` 或 `/btw` 命令成功后**自动弹出面板**，新子代理立即可见
- 手动刷新按钮、错误重试、空态提示一应俱全

实现说明：面板是插件自带的浮动覆盖层（`position: fixed` 右缘），不修改主仓库布局；数据直接复用运行时的子代理 catalog（`sessions.list.subagentsByParent`），不新增 RPC。
...
4. `/side 你好` 成功后，标题行侧链按钮自动亮起、右侧面板弹出并列出该子代理；点行可跳回。
