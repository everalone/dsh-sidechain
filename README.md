...
当前版本适配公开版 DSH npm `0.0.1-rc.5`、`0.1.0-rc.6`、`0.1.0-rc.7`、`0.1.0-rc.8` 和 `0.1.1-rc.1`。在已验证的 `0.1.1-rc.1` 中，`/side` 与 `/btw` 也会转发命令输入中的图片附件，并在侧栏显示持久化图片。
...
`pnpm check` 依次执行类型检查、测试和构建。开发依赖固定到最新验证版 DSH `0.1.1-rc.1`，不需要本机存在 DSH 源码 checkout。
