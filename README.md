...
- **面板列出当前会话的全部直接子代理**：包含你自己的 `/side`/`/btw` 线程，也可能包含平台自带的常驻侧线程（Harness 为每个主会话自动挂的 background 侧代理，标签为 "Side conversation"）——它由平台创建并在后台自主工作，所以看起来"自己在说话"；面板只在**点击选中**某线程时才流式显示其对话，不会自动打开常驻代理
