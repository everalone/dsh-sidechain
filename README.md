...
| **一次性侧问** | `/btw <问题>` | 单个问题、单个回答，答案内联返回主会话，用完即止 | Claude 式单次对话 |

> 注：Codex 上 `/btw` 与 `/side` 行为相同（均为持续性侧会话）；本插件刻意区分二者——`/side` 为持续性侧线程，`/btw` 为一次性问答。
...
- `/side` —— 创建空侧线程（等待你的第一个问题）
- `/side list` —— 列出本会话的全部直接子代理
- `/btw 这个目录下哪个文件最大？` —— 一次性侧问：答案内联返回；如需多轮，请用 `/side`
